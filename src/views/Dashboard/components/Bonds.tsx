      ...
      <div className="grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr'}}>
        ...
        <div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
            Purchase BBond <StyledButton onClick={(val)=>{
              onPresentP();
              
              }} disabled={!bondStat || isBondRedeemable}>Purchase</StyledButton>
            <span style={{display:(!bondStat || isBondRedeemable)?'inline':'none'}}>Bomb is over peg</span>
          </div>
          <hr />
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
            Redeem BBond <StyledButton onClick={(val)=>{
              onPresentR();
            }} disabled={!bondStat || bondBalance.eq(0) || !isBondRedeemable}>Redeem</StyledButton>
            <span style={{display:(!bondStat || bondBalance.eq(0) || !isBondRedeemable)?'inline':'none'}}> Enabled when 10,000 BOMB &gt; ${BOND_REDEEM_PRICE}BTC</span>
          </div>
        </div>
      </div>
    </Styleddiv>
